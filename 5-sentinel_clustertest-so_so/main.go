...
func main() {
	...
	redisClient := redis.NewFailoverClusterClient(&redis.FailoverOptions{
		MasterName:         "redis2",
		SentinelAddrs:      []string{"10.10.1.11:26379", "10.10.1.12:26379", "10.10.1.13:26379"},
	...
	clusterInfo, _ := redisClient.ClusterInfo(ctx).Result()
	clusterNodes, _ := redisClient.ClusterNodes(ctx).Result()
	//info, _ := redisClient.Info(ctx).Result()
	...
	str, err := redisClient.Set(ctx, "foo", "kiss my bar ass", time.Second * 100).Result()
	log.Println(str, err)

	res, err := redisClient.Get(ctx, "foo").Result()
	log.Println(res, err)
